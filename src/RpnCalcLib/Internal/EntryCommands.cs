...
namespace Menees.RpnCalc.Internal
{
    internal class EntryCommands : Commands
    {
        ...
        public void AppendToEntryLine(Command cmd, int offsetFromTop)
        {
            ...
            string valueText = value.GetEntryValue(Calc);

            string entryLine = Calc.EntryLine;
            if (string.IsNullOrEmpty(entryLine))
            {
                Calc.EntryLine = valueText;
            }
            else
            {
                Calc.EntryLine += " " + valueText;
        ...
        public void Edit(Command cmd)
        {
            ...
            if (string.IsNullOrEmpty(entryLine))
            {
                ...
                Calc.EntryLine = value.GetEntryValue(Calc);
                cmd.Commit();
            }
            else
            {
        ...
        public EntryLineParser Enter(Command cmd)
        {
            ...
            if (string.IsNullOrEmpty(entryLine))
            {
                ...
                m_stackCommands.Dup(cmd);

                return null;
            }
            else
            {
                EntryLineParser parser = new EntryLineParser(Calc, entryLine);
                if (parser.HasError)
                {
                    Calc.ErrorMessage = parser.ErrorMessage;
                    cmd.Cancel();
                }
                ...
                Calc.EntryLineHistory.Add(entryLine);
